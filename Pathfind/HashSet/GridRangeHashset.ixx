    ...
    namespace HashSet {
        ...
        public:        
            inline GridRangeBaseHashset(const SG_Grid::Point& centrePoint, const SG_Grid::coordinate_t& distance, const pathfindGrid_t& within, insideArena_t& arena):
                arena(arena)
                { 
                origin = SG_Grid::Point(
                    std::max(0, centrePoint.x() - distance),
                    std::max(0, centrePoint.y() - distance)
                );
            ...
            inline SG_Grid::Point calcHash(const SG_Grid::Point& toHash) {return toHash-origin;}
