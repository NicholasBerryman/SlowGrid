...
void testOnStack(){
    ...
    SG_Pathfind::PriorityQueue::HashMapBinaryHeap q(arena, grid, SG_Grid::Point(5,5), 4, 4);

    q.insert(SG_Grid::Point(6,6),1);
    q.insert(SG_Grid::Point(6,5),1);
    q.insert(SG_Grid::Point(7,7),3);

    assert(q.valueAt(1)   == SG_Grid::Point(6,5) || q.valueAt(1) == SG_Grid::Point(6,6));
    assert(q.valueAt(3)   == SG_Grid::Point(7,7));


    assert(q.length() == 3);
    LOGGER_ASSERT_ERROR( q.valueAt(4); )
