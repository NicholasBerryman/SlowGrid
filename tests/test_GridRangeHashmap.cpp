...
void testOnStack(){
    ...
    LOGGER_ASSERT_ERROR( (void) (hash.contains(SG_Grid::Point(1,2)) == false); )
...
void testGridEdge(){
    ...
    SG_Pathfind::HashMap::GridRangeHashMap<decltype(arena), T, false> hash(arena, grid, SG_Grid::Point(11,11), 3);
    hash.insert({10,10}, 3);
    hash.insert({11,11}, 9);
    hash.insert({8,8},10);
    ...
    SG_Pathfind::HashMap::GridRangeHashMap<decltype(arena), T, false> hash3(arena, grid, SG_Grid::Point(11,5), 3);
    hash3.insert({10,5}, 3);
    hash3.insert({11,5}, 9);
    hash3.insert({8,5},10);
